...
const count = (target, input) => {
  return input.reduce((acc, arr) => {return acc + arr.filter(number => number === target).length}, 0)
};
...
const totalSum = (input) => {
  let rowValues = new Array(input.length).fill(0);
...
let findMaleAndFemale = (data) => {
  return data.filter(Object => Object.gender === 'female' || Object.gender === 'male').map(Object => Object.name).join(' and ');
};
...
let findShortest = (data) => {
  return data.reduce((acc, character) => {
    if (acc.name.length < character.name.length) {return acc;}
